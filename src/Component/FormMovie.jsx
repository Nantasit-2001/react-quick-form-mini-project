...
import SurveyResult from "./FormSucceed";
function FormMovie () {
    const [selectedOption, setSelectedOption] = useState("");
    const [textInput, setTextInput] = useState({});
    const [dataError,setDataError] = useState({});
    const [formSucceedInput,setFormSucceedInput] = useState(false)

    const BackToForm=()=>{
        resetValue();
        setFormSucceedInput(false);
    }
    const ThenSubmit = (event) => {
        event.preventDefault();
        setDataError({})
        CheckCorrectValue()?setFormSucceedInput(true):console.log("❌ => ", dataError)
    };
    ...
    return(
        ...
        <section className="w-full max-w-md shadow- mx-2">
            ...
            {formSucceedInput?
                (<SurveyResult textInput={textInput} selectedOption={selectedOption} BackToForm={BackToForm} />)
                : // form
                <section>
